/* ============================================================
   Permiso eliminar versiones borrador — sgd/documentos
   Ejecutar: mysql savid < database/migrations/20260609_sgd_version_eliminar_permiso.sql
============================================================ */

INSERT INTO accion (codigo, nombre, estado_id)
SELECT 'version_eliminar', 'Eliminar versión (borrador/obsoleta)', 1
WHERE NOT EXISTS (SELECT 1 FROM accion WHERE codigo = 'version_eliminar' LIMIT 1);

INSERT INTO item_accion (item_id, accion_id, estado_id)
SELECT i.id, a.id, 1
FROM item i
INNER JOIN accion a ON a.codigo = 'version_eliminar'
